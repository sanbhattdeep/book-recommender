"""
Analyze the frozen 30-case unseen validation run without scikit-learn/scipy.

This is a drop-in replacement for evals/analyze_judge_validation.py when
Windows Application Control blocks SciPy native DLLs.

Usage:
    uv run python evals/analyze_judge_validation.py `
      --run evals/runs/semantic_relevance_validation/<RUN_ID>
"""
...
EXPECTED_EVALUATION_DATASET_ROLE = "validation"
EXPECTED_EVALUATION_SCOPE = "unseen_validation"
...
def resolve_dataset(metadata: dict) -> Path:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"Validation dataset recorded in metadata was not found: {path}"
        )
    return path
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze frozen v0.15 unseen validation run."
    )
    ...
    if mismatches:
        raise ValueError(
            "Run is not the expected frozen v0.15 validation run:\n- "
            + "\n- ".join(mismatches)
        )
    ...
    if len(gold) != 30:
        raise ValueError(
            f"Expected 30 validation gold cases, found {len(gold)}."
        )
    ...
    if len(comparison) != 30:
        ...
        raise ValueError(
            f"Validation run is incomplete: compared {len(comparison)}/30. "
    ...
    comparison.to_csv(
        run_dir / "human_vs_judge.validation.csv",
        index=False,
        encoding="utf-8",
    )
    matrix.to_csv(
        run_dir / "confusion_matrix.validation.csv",
        encoding="utf-8",
    )
    agreement.to_csv(
        run_dir / "agreement_by_human_score.validation.csv",
        index=False,
        encoding="utf-8",
    )
    largest.to_csv(
        run_dir / "largest_disagreements.validation.csv",
        index=False,
        encoding="utf-8",
    )
    (
    ...
    print()
    print("Pre-registered acceptance checks")
    print("--------------------------------")
