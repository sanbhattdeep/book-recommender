"""Build the 90-case v0.20 development set from already-consumed data only.
...
OUT = DATASETS / "semantic_relevance_v0.20_development.v2.0.0.csv"
MANIFEST = DATASETS / "semantic_relevance_v0.20_development_manifest.v1.0.0.json"
...
def main() -> None:
    ...
    combined["dataset_version"] = "2.0.0"
    combined["rubric_version"] = "0.1.0"

    if len(combined) != 90 or combined["case_id"].nunique() != 90:
        raise ValueError("v0.20 development output must contain 90 unique cases.")
    if combined["isbn13"].nunique() != 90:
        raise ValueError("v0.20 development output must contain 90 unique ISBNs.")
    ...
    print("v0.20 development dataset built")
    print("--------------------------------")
