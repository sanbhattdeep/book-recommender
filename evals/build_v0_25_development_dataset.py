"""Build the 90-case v0.25 development set from already-consumed data only.
...
OUT = DATASETS / "semantic_relevance_v0.25_development.v2.1.0.csv"
MANIFEST = DATASETS / "semantic_relevance_v0.25_development_manifest.v1.0.0.json"
...
def main() -> None:
    ...
    if len(combined) != 90 or combined["case_id"].nunique() != 90:
        raise ValueError("v0.25 development output must contain 90 unique cases.")
    if combined["isbn13"].nunique() != 90:
        raise ValueError("v0.25 development output must contain 90 unique ISBNs.")
    ...
    print("v0.25 development dataset built")
    print("--------------------------------")
