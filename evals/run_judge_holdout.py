"""
Run frozen Semantic Recommendation Relevance Judge v0.15.0 exactly once on the final unseen holdout.
...
Targeted semantic diagnostics are intentionally disabled for the final holdout.


Final holdout run:

    uv run python evals/run_judge_holdout.py

Resume an interrupted run:

    uv run python evals/run_judge_holdout.py `
      --resume evals/runs/semantic_relevance_final_holdout/<RUN_ID>
...
-----------------
    ...
    definition-aware per-facet candidate evidence selection
            ↓
    isolated definition-aware evidence-vs-facet verification
            ↓
...
Human holdout labels remain blind to every judge stage.
"""
...
from check_holdout_integrity import validate_holdout_contract
...
EVALUATION_DATASET_ROLE = "final_holdout"
EVALUATION_SCOPE = "unseen_final_holdout"
...
#
#     <repo>/evals/run_judge_holdout.py
#
...
DATASET_FILE = (
    EVALS_DIR
    / "datasets"
    / f"semantic_relevance_final_holdout.v{EVALUATION_DATASET_VERSION}.csv"
)
...
RUNS_DIR = (
    EVALS_DIR
    / "runs"
    / "semantic_relevance_final_holdout"
)
...
def load_facet_specs(
    ...
    for raw_query in raw_queries:
        ...
        # This deterministic final holdout catches duplicate facet IDs and facet
...
# =============================================================================
# Input/provenance final holdout
# =============================================================================
...
def validate_inputs(
    ...
    #
    # Human fields are validated here because this final holdout was labelled
    # before judge execution, but they are NEVER placed in the judge prompt.
    # -------------------------------------------------------------------------
    required_columns = {
        "case_id",
        "query_id",
    ...
    if len(dataset) != 30:
        raise ValueError(
            "The frozen final holdout must contain exactly 30 cases; "
            f"found {len(dataset)}."
        )

    if not dataset["case_id"].astype(str).str.startswith("U_").all():
        raise ValueError(
            "Final-holdout case IDs must use the frozen unseen-pool U_ prefix."
        )
    ...
    if not dataset[
        ...
        raise ValueError(
            "Final holdout contains missing human_score values."
        )
    ...
    if set(
        ...
        raise ValueError(
            "Final-holdout dataset rows do not all declare "
...
def git_commit_sha() -> str | None:
    ...
    try:
        ...
        # Missing Git metadata should not prevent local final-holdout evaluation.
        return None
...
def validate_resume_metadata(
    ...
    if mismatches:
        raise ValueError(
            ...
            "the frozen v0.15 final-holdout runner:\n- "
            + "\n- ".join(
                mismatches
            )
        )
...
def write_metadata(
    ...
    existing_metadata_file = metadata_file_for(run_dir)
    if existing_metadata_file.exists():
        try:
            existing_metadata = load_json(existing_metadata_file)
        except Exception:
            existing_metadata = {}
    ...
    metadata = {
        ...
        "final_holdout_single_evaluation": True,
        "targeted_semantic_diagnostics_allowed": False,
        "transport_recovery_requires_recorded_failure": True,
        "holdout_integrity_checked_before_execution": True,
        ...
        # Transport overrides are operational provenance and must survive
        # subsequent normal runner metadata rewrites.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "30-case final unseen holdout using local Ollama."
        )
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Operational recovery only. Requires --resume and a matching "
            "last_failure.json written by this holdout runner. This is not "
            "a targeted diagnostic option."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Resume the same frozen final-holdout run directory. "
            "Do not resume with changed judge artifacts."
        ),
    )
    ...
    if args.recover_failed_case is not None and args.resume is None:
        raise ValueError(
            "--recover-failed-case requires --resume for the already-created "
            "final-holdout run."
        )

    # Integrity is checked before loading any case or making any model call.
    validate_holdout_contract(quiet=True)
    ...
    # Final holdout forbids arbitrary targeted execution. The only allowed
    # single-case path is recovery of the exact case that previously failed
    # transport/execution and was recorded in last_failure.json.
    if args.recover_failed_case is not None:
        failure_file = run_dir / "last_failure.json"
        if not failure_file.exists():
            raise ValueError(
                "Recovery requested but last_failure.json does not exist."
            )
        failure = load_json(failure_file)
        failed_case_id = str(failure.get("case_id"))
        requested_case_id = str(args.recover_failed_case)
        if requested_case_id != failed_case_id:
            raise ValueError(
                "Recovery case does not match the actually failed pending case: "
                f"requested={requested_case_id!r}, failed={failed_case_id!r}."
            )
        if requested_case_id in completed_case_ids:
            raise ValueError(
                f"{requested_case_id} is already complete; refusing recovery rerun."
            )
        remaining_cases = remaining_cases[
            remaining_cases["case_id"].astype(str) == requested_case_id
        ].copy()
        if len(remaining_cases) != 1:
            raise ValueError(
                "Failed-case recovery did not resolve to exactly one pending row."
            )
    ...
    try:
        ...
        # Persist the exact failed pending case without exposing any human gold.
        # Direct-transport recovery is allowed only for this recorded case.
