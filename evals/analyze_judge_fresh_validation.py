...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--run',required=True); args=p.parse_args(); validate_split_contract(require_release=False)
    run_dir=resolve_run_dir(args.run)
    summary=analyze_run(run_dir,'fresh_unseen_validation','fresh_unseen_validation','fresh_validation')
    print(); print('Validation gate: '+('PASS — eligible for explicit holdout release' if summary['all_reference_thresholds_met'] else 'FAIL — final holdout remains locked'))
