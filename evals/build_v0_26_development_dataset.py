"""Build the 90-case v0.26 development set from already-consumed data only.
...
def main() -> None:
    ...
    if len(combined) != 90 or combined["case_id"].nunique() != 90:
        raise ValueError("v0.26 development output must contain 90 unique cases.")
    if combined["isbn13"].nunique() != 90:
        raise ValueError("v0.26 development output must contain 90 unique ISBNs.")
    ...
    print("v0.26 development dataset built")
    print("--------------------------------")
